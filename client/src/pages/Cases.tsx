...
export default function Cases() {
  ...
  // ====== AQUI Estava a duplicidade ======
  // Antes: incluía 'atrasado' dentro de "pending"
  // const pendingCases = filteredCases.filter((c) => c.status === 'novo' || c.status === 'pendente' || c.status === 'atrasado');
  // FIX: pendente = 'novo' OU 'pendente' (NÃO inclui 'atrasado')
  const pendingCases = filteredCases.filter((c: CaseWithRelations) => c.status === 'novo' || c.status === 'pendente'); // FIX
